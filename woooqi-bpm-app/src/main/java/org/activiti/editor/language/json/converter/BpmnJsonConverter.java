...
public class BpmnJsonConverter implements EditorJsonConstants, StencilConstants, ActivityProcessor {
	...
	private static final String STENCIL_TASK_SIGN = "SignTask";
        ...
        for (String edgeId : edgeMap.keySet()) {
            ...
            if (!"".equals(edgeNode.get("properties").get("name"))) {
                bpmnModel.addLabelGraphicInfo(edgeId,
                		createGraphicInfo((graphicInfoList.get(0).getX() + graphicInfoList.get(1).getX())/2 ,(graphicInfoList.get(0).getY() + graphicInfoList.get(1).getY())/2));
            }
        }
    }
