...
public class DefaultProcessDiagramGenerator implements ProcessDiagramGenerator {
  ...
  public DefaultProcessDiagramGenerator(final double scaleFactor) {
    ...
    // sign task
    activityDrawInstructions.put(SignTask.class, new ActivityDrawInstruction() {

      public void draw(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode) {
        GraphicInfo graphicInfo = bpmnModel.getGraphicInfo(flowNode.getId());
        processDiagramCanvas.drawSignTask(flowNode.getName(), graphicInfo, scaleFactor);
      }
    });
  ...
  public InputStream generateDiagram(BpmnModel bpmnModel, List<String> highLightedActivities, List<String> highLightedFlows,List<String> activeActityIds,List<Map<String, Object>> turnTransitionActivity) {
	  return generateProcessDiagram(bpmnModel, highLightedActivities, highLightedFlows,activeActityIds,turnTransitionActivity).generateImage("PNG");
  }
  ...
  protected DefaultProcessDiagramCanvas generateProcessDiagram(BpmnModel bpmnModel, String imageType, 
    ...
    for (Process process: bpmnModel.getProcesses()) {
      for (FlowNode flowNode : process.findFlowElementsOfType(FlowNode.class)) {
        drawActivity(processDiagramCanvas, bpmnModel, flowNode, highLightedActivities, highLightedFlows, false);
      }
    }
  ...
  protected DefaultProcessDiagramCanvas generateProcessDiagram(BpmnModel bpmnModel,List<String> highLightedActivities, List<String> highLightedFlows,List<String> activeActivityIds,List<Map<String, Object>> turnTransitionActivity) {
	  	
	  	prepareBpmnModel(bpmnModel);
	    
	    DefaultProcessDiagramCanvas processDiagramCanvas = initProcessDiagramCanvas(bpmnModel, "PNG", "宋体", "宋体", "宋体", null);
	    ...
	    for (Process process: bpmnModel.getProcesses()) {
	      for (FlowNode flowNode : process.findFlowElementsOfType(FlowNode.class)) {
	    	  if(activeActivityIds.contains(flowNode.getId())){
	    		  drawActivity(processDiagramCanvas, bpmnModel, flowNode, highLightedActivities, highLightedFlows, true);
	    	  }else{
	    		  drawActivity(processDiagramCanvas, bpmnModel, flowNode, highLightedActivities, highLightedFlows, false);
	    	  }
	      }
	    ...
	    //Draw turnTransition Activity
	    drawTransitionActivity(processDiagramCanvas,bpmnModel,turnTransitionActivity);
  ...
  protected void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel,FlowNode flowNode, List<String> highLightedActivities, List<String> highLightedFlows, Boolean isActive) {
    ...
    if (drawInstruction != null) {
      ...
      if (highLightedActivities.contains(flowNode.getId())) {
    	  if(isActive){
    		  drawHighLight(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()),flowNode.getClass().getSimpleName()+":Active");
    	  }else{
    		  drawHighLight(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()),flowNode.getClass().getSimpleName());
    	  }
      }
    ...
    for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
      ...
      if (graphicInfoList != null && graphicInfoList.size() > 0) {
        ...
        processDiagramCanvas.drawSequenceflow(xPoints, yPoints, drawConditionalIndicator, isDefault, highLighted, 1.0);
    ...
    if (flowNode instanceof FlowElementsContainer) {
      for (FlowElement nestedFlowElement : ((FlowElementsContainer) flowNode).getFlowElements()) {
        if (nestedFlowElement instanceof FlowNode) {
          drawActivity(processDiagramCanvas, bpmnModel, (FlowNode) nestedFlowElement,highLightedActivities, highLightedFlows, isActive);
        }
      }
    }
  }
  
  private void drawTransitionActivity(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel,List<Map<String, Object>> transitionRecords) {
	for(int i = 0;i<transitionRecords.size();i++){
		int xPoints[]= new int[2];
	    int yPoints[]= new int[2];
		Map<String, Object> transitionRecord = transitionRecords.get(i);
		
		String sourceId = transitionRecord.get("sourceId").toString();
	    String targetId = transitionRecord.get("targetId").toString();
	    String type = transitionRecord.get("type").toString();
	    if(type.equals("back")){
		    FlowNode sourceFlowNode = (FlowNode) bpmnModel.getFlowElement(sourceId);
		    List<SequenceFlow> incomingFlows = sourceFlowNode.getIncomingFlows();
		    SequenceFlow sourceSequenceFlow = incomingFlows.get(0);
		    
		    FlowNode targetFlowNode = (FlowNode) bpmnModel.getFlowElement(targetId);
		    List<SequenceFlow> outgoingFlows = targetFlowNode.getOutgoingFlows();
		    SequenceFlow targetSequenceFlow = outgoingFlows.get(0);
		    
		    List<GraphicInfo> sourceGraphicInfos = bpmnModel.getFlowLocationGraphicInfo(sourceSequenceFlow.getId());
		    List<GraphicInfo> targetGraphicInfos = bpmnModel.getFlowLocationGraphicInfo(targetSequenceFlow.getId());
		    
		    xPoints[0] = (int) sourceGraphicInfos.get(sourceGraphicInfos.size()-1).getX();
	        yPoints[0] = (int) sourceGraphicInfos.get(sourceGraphicInfos.size()-1).getY();
	        
		    xPoints[1] = (int) targetGraphicInfos.get(0).getX();
	        yPoints[1] = (int) targetGraphicInfos.get(0).getY();
	    }else if(type.equals("jump")){
	    	FlowNode sourceFlowNode = (FlowNode) bpmnModel.getFlowElement(sourceId);
		    List<SequenceFlow> outgoingFlows = sourceFlowNode.getOutgoingFlows();
		    SequenceFlow sourceSequenceFlow = outgoingFlows.get(0);
		    
		    FlowNode targetFlowNode = (FlowNode) bpmnModel.getFlowElement(targetId);
		    List<SequenceFlow> incomingFlows = targetFlowNode.getIncomingFlows();
		    SequenceFlow targetSequenceFlow = incomingFlows.get(0);
		    
		    List<GraphicInfo> sourceGraphicInfos = bpmnModel.getFlowLocationGraphicInfo(sourceSequenceFlow.getId());
		    List<GraphicInfo> targetGraphicInfos = bpmnModel.getFlowLocationGraphicInfo(targetSequenceFlow.getId());
		    
		    xPoints[0] = (int) sourceGraphicInfos.get(0).getX();
	        yPoints[0] = (int) sourceGraphicInfos.get(0).getY();
	        
		    xPoints[1] = (int) targetGraphicInfos.get(targetGraphicInfos.size()-1).getX();
	        yPoints[1] = (int) targetGraphicInfos.get(targetGraphicInfos.size()-1).getY();
	    }
        
        
        String angle = getAngle(xPoints[0],yPoints[0],xPoints[1],yPoints[1]);
        if(angle.equals("y")){
        	yPoints[0] = yPoints[0] - 10;
        	yPoints[1] = yPoints[1] - 10;
        }else if(angle.equals("x")){
        	xPoints[0] = xPoints[0] + 10;
        	xPoints[1] = xPoints[1] + 10;
        }
	  
	   processDiagramCanvas.drawTransitionSequenceflow(xPoints, yPoints, type);
	} 
  }
  
  public static String getAngle(int x1,int y1,int x2,int y2){
	  String ret = "";
	  double dx = (x2 - x1);
	  double dy = (y2 - y1);
	  if(dy == 0){
		  ...
		  int angle = (int) (Math.atan(dx/dy)*180/Math.PI);
		  if(dx > 0 && dy < 0){
			  angle = angle+90;
		  }else if(dx < 0 && dy < 0){
			  angle = angle+90;
...
/**
  ...
  private static void drawHighLight(DefaultProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo,String flowNode) {
	  if(flowNode.endsWith("Gateway")){
		  processDiagramCanvas.drawGatewayHighLight((int) graphicInfo.getX(), (int) graphicInfo.getY(), (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight());
	  }else{
		  if(flowNode.endsWith(":Active")){
			  processDiagramCanvas.drawActiveHighLight((int) graphicInfo.getX(), (int) graphicInfo.getY(), (int) graphicInfo.getWidth(), (int) graphicInfo.getHeight());
		  }else{
