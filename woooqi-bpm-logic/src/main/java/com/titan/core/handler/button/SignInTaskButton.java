...
@Component
public class SignInTaskButton implements BaseButton{
	...
	public boolean support(String buttonCode) {
		if(buttonCode.toLowerCase().equals("signintask")){
			return true;
		}
		return false;
	}
	...
	public void handler(String taskId,Map<String, Object> before) {
		try {
			...
			int sign = runtimeService.getVariable(task.getExecutionId(), "nrOfSignins")==null?0:Integer.parseInt(runtimeService.getVariable(task.getExecutionId(), "nrOfSignins").toString());
			sign = sign + 1;
			runtimeService.setVariable(task.getExecutionId(), "nrOfSignins", sign);
